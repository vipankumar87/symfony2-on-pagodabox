...
Don't forget to `$ php app/console clear:cache --env=prod` if you get an error.
...
By using the `-u` flag in the last command `pagoda` became the default remote for the current branch (currently master). From now on each time you push commits (to pagoda, in the master branch) your app will rebuild and redeploy — pretty powerful!
...
```
To simplify local configuration you can optionally omit the `SYMFONY__OTHER__SECRET` environment var and hardcode a secret in `parameters.yml` with this one-liner:
...
Skipping the interactive prompts will create `parameters.yml` with the defaults in `parameters.yml.dist`.
